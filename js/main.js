...
var name = "";
var content = "";
var url = "https://www.example.com/index/";
...
$(function(){
    ...
    chrome.storage.local.set({'name': name,"guid":guid,'csrf':csrf}, function() {});
    ...
    $(document).on("submit","form.form-edit-todo",function(e){
        ...
        if(params.fix_user || params.status ){
            if(edit_bug(content,params) && params.status == 1){
                _nums_incr(1);
                window.setTimeout(delay_bind,5000);
            }
        }
    ...
    delay_bind();
    function delay_bind(){
        $("div.simple-checkbox").each(function(i,e){
            if($(e).data("delay_bind")!=1){
                $(e).click(function(){
                    var div = $(this),content = div.next().find("span.raw").text(),params = {};
